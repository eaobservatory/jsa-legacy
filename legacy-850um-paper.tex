...
legacy reductions are envisioned as providing a uniform, standardised
...
data from \hl{XXXX to YYYY}\footnote{\hl{cite web page}}. This paper
presents the expanded and updated LR2 release including SCUBA-2
850\micron data form \hl{XXXX to YYYY}.

This paper presents the second release of the 850\,\um\ SCUBA-2
continuum observations. Planned future releases are the 450\,\um\
SCUBA-2 observations and HARP spectral cubes, as well as regular
updates to the 850\,\um\ release.
...
\begin{figure*}
  ...
  \caption{\hl{All-sky noise map for this SCUBA-2 850\,\um{} legacy
...
Observations from earlier than 2011 February 2 were not included, as these were
...
observations from within the appropriate time period which failed
...
areas \citep{Holland2013}.  The scanning speed is not the
same between these scan patterns; this will cause some inconsistency
in the scales of emission detected in the reduced maps, as SCUBA-2
observations are sensitive to different size scales of emission at
different speeds/scan. However, as our reduction has been
...
noise is \hl{0.35 m\jyas}. Table~\ref{tab:noises} shows the area of
...
less than or equal to a given limit. We have give the noise in \jyas\
and Jy\,beam$^{-1}$ using self-derived FCFs (see section
\ref{sec:calib} for details on this
process).

% For convenience of those readers
% most used to Jy\,beam\textsuperscript{$-1$}, we have provided a very
% approximate equivalent flux density in those units by dividing by our
% derived arcsecond FCF (2.48\,\jyas\,pW$^{-2}$) and multiplying by the
% JCMT `standard' beam FCF (537 Jy\,beam$^{-1}$\,pW$^{-1}$.

% Please note
% that a beam FCF has not been derived for this dataset, so we do not
% recommend using this calibration when extracting fluxes as a self
% derived FCF would probably have a systematic offset of a few percent
% from the `standard' values, (as shown for the arcsecond FCF in
% Section~\ref{sec:calib}).


\begin{deluxetable}{lDDD}
  \tablecaption{\hl{Areas of the release with an RMS noise less than or
...
\begin{figure}
  ...
  \caption{\hl{Histogram of the RMS noise on each pixel in the co-adds. The
...
\begin{deluxetable}{rc r c c p{2cm}}
  \tablecaption{\todo{Reorder table appropriately!} \hl{The twenty co-added tiles that contain regions with a
...
(M13AH07B).\note{Was this published?}. The noise maps
...
\begin{figure*}
  ...
  \caption{The coadded products towards Tile\,30318, containing the
    ...
    emission map. Top right: the coadded noise map, exhibiting the
    ...
    outlined on the coadded emission map. Bottom right: The detected
...
\begin{figure*}
  ...
  \caption{Examples of the co-add, noise map, and extents towards the
    ...
    easily seen. \todo{HALP: Should contours in bottom left image be narrower?}}
  \label{fig:crl618}
\end{figure*}
...
\begin{figure*}
  ...
  \caption{Tile\,27258 showing an extrememly deep co-add towards a
    ...
    triangles]{Geach2016}. \todo{HALP: Comment on strcuture visible in
      background of deepest part of coadd?}}
  \label{fig:t27258}
\end{figure*}
...
the structure of the astronomical emission observed. \todo{Should this
  be inluded? HALP votes yes. For example, a
...
detail on the map maker, see \citet{Chapin2013}. \note{HALP suggests
  also citing Stever Mairs 2015 here?}.
...
first data release (2011 to 2013, not including theCLS) only used a
...
process. \note{Does it need these extra images? Examples of some
  observations flagged as BAD can be seen in Figure
  XX. HALP votes no.}
...
\todo{Do we want some power spectra of this? not sure if they'd be
  very illuminating. HALP votes no!}
...
the JMCT-LR makemap configuration. They found from simulations that
...
and derived FCFs from our calibration observations. A very bright
source (such as Uranus) will have a larger area included in the AST
mask due to its higher signal-to-noise at a given RMS, when compared
...
growth for Uranus as opposed to CRL\,618 and CRL\,2688 (see in figure
...
difference with the 60\arcsec\ source aperture used here, but larger
when looking at larger apertures. \hl{Give table showing the
  difference at various apertures?} For reference, Uranus has an
...
source (based on their brightness).  This will then cause different
proportions of the flux in the source to be recovered in the final
map, due to the differing recovery in \texttt{makemap} reductions of
flux within and without the beam. 
...
has an average peak brightness of \hl{0.009} pW in these observations)
to roughly the brightness of Uranus (an average peak of \hl{0.11}
...
will have a smaller area in their SNR-defined AST mask, and will thus
see a variation in the resultant FCF. As the major determiner of the
noise is the 850\,\um\ transmission of the observation, this will lead
to a variation in derived FCF with both weather and elevation. Figure
...
reference, we also give the variation in derived arcsecond FCFs with
...
using a single FCF number. As Uranus being far brighter than
...
iterative clipping to exclude FCF values 5-$\sigma$ away from the mean
to remove extreme outliers (which may be due to poor instrumental
performance, bad weather, day time observing or other affects.) See
figure \ref{fig:fcfhist} for the full distributions. The final average
values are:
...
We used the FellWalker algorithm as implemented in \texttt{cupid} on an SNR map
...
these points by running the FellWalker algorithm (from \texttt{CUPID}) on the
