...
\begin{abstract}
  ...
  of $~$1 degree a side, using the HPX projection with pixels of size
  ...
  5828 hours of this, covering 1356 square degrees of the sky. The
  co-adds have been calibrated into units of m\jyas\ using a
  \hl{self-derived} flux conversion factor. Contiguous regions of emission
  were detected at a better than 5-$\sigma$ level in 740 tiles,
  covering in total 1.4 square degrees, or 0.10 percent of the
...
full science mode since \hl{February} 2011.
...
standardized reduction, co-addition, and emission detection of all
publicly available observations, regularly updated as more
observations become public. The aim has been to produce uniformly
reduced high-quality co-added maps that required as little checking as
possible by eye, and which would allow astronomers to easily discover
which regions have been observed, the noise levels in existing
regions, and where there are clear detections. Previously, the SCUBA
Legacy Catalog \citep{DiFrancesco2008} presented uniformly reduced
co-adds and catalogs of much of the public SCUBA data, and this resource has been
very widely used.

The first product of the legacy release project, the JCMT legacy
release (LR1) was made public in \hl{September} 2015, and included
...
hosted at the Canadian Astronomy Data Centre (CADC) \hl{Is there a
  citation for CADC?}. This archive also hosts our proprietary (with
authentication) and public raw observations and our standard pipeline
reduced products. Unlike the LR, the normal reductions do not include
full coadds of data taken on different nights. In addition,
observations from different projects may have been reduced with
different configurations and can not be naively coadded.
...
September 2015, using the same configuration and co-add criteria as
...
all-sky noise map of the co-adds in this release, showing the average
...
\ref{sec:QA}). Co-adds were then made for every tile with data
...
Section~\ref{sec:calib}). For every co-added tile, the recipe
...
speeds. However, our reduction has been extremely conservative
...
observing time, towards 4403 distinct tiles. The co-added observations
include 12404 science and calibration observations which passed QA
(5828 hours or 242.8 days) towards 4151 tiles. These co-adds were made
...
\ref{tab:typesobs}. The co-adds include 1.69 gigapixels of valid data,
...
Emission detection was carried out on all of the co-added tiles, and
this resulted in detections in 1.37 square degrees (4.17e-4
steradians, or 0.10\% of the area observed).
...
area of our co-adds which has a noise level below various common
...
noise in \jyas\ and Jy\,beam$^{-1}$ using \hl{self-derived} FCFs (see
Section \ref{sec:calib} for details on this process).



\subsection{The deepest maps}
Twenty of our co-added tiles contain pixels with a noise better
0.0043\,m\jyas{}, corresponding to 1\,m\jybm. These include our
three most common standard SCUBA-2 calibrators (CRL\,618, CRL\,2688
and Arp\,220), and several deep cosmology fields that were observed
...
wavelengths. Figure~\ref{fig:g34-3} shows our co-added tile, noise
map, extent catalog, and peak catalog towards this source. This co-add
...
(M13AH07B). The noise maps (Figure~\ref{fig:g34-3}, 2nd figure) for
this co-add shows the characteristic CV\_DAISY scan pattern towards
the central source, with two overlapping small PONG observations
covering a wider extent. This field also illustrates the higher noise
we see towards bright sources -- clearly seen in the towards the
central source and the bright points in the filament extending towards
the top left. Examination of the extent and peak catalogs, however,
(shown below) indicates that this not prevent a detection of the
...
signal-to-noise ratio (SNR) of $>5$. \hl{Description of why we see
  higher noise? Or leave for DR? currently not gone into anywhere in
  the paper.}
...
\begin{figure*}
  ...
  \caption{The co-added products towards Tile\,30318, containing the
    high-mass star-forming source G034.27+0.15. Upper left: the co-added
    emission map. Upper right: the co-added noise map, exhibiting the
    ...
    outlined on the co-added emission map. Lower right: the detected
...
SCUBA-2. As such, this co-add includes 923 observations using 51 hours
of observing time across the small CV\_DAISY area, shown in
...
around the central position shows clearly a variety of point
sources detected in this deep co-add. In addition, it can readily be
seen by eye that there appear to be additional sources that are missed
by our cataloging procedure due to the negative bowling around the
very bright source. Horizontal and vertical cut throughs are shown to
help visualise this bowling.
...
\begin{figure*}
  ...
  \caption{Examples of the co-add, noise map, and extents towards the
    ...
    and 1244. This co-add contains data from 923 observations (51\,hrs
...
patterns (both PONG and CV\_DAISY observations are included in this
tile); no attempt to correct for this was made. \hl{ In these deep fields,
...
\begin{figure*}
  ...
  \caption{Tile\,27258 showing an extremely deep co-add towards a
    ...
    taken by JCMT Cosmology Legacy survey and several UH cosmology
    projects. Parts of these data were published in
    \citet{Casey2013,Chen2013,Chen2013a,Geach2016}. Top left: emission
    map of the entire co-added tile. A white contour indicates the
    extremely deep area with a noise < 5\,\jyas\, occurring because of
    many CV\_DAISY observations of this area.  Top right: noise map of
...
twelve facets, and then recursively divides these cells in four at
each higher resolution level.  \hl{There are two standard numbering
  schemes used with HEALPix}\footnote{Remove?}, of which the
``nested'' scheme has been used to label tiles in this release.  \hl{
  This scheme allows simpler conversion to and from positions in the
  HPX projection}, as well as compatibility with Virtual Observatory
systems such as MOC \citep[Multi-Order
...
the trade off is that the pixels are not generally square: they can vary in
width and length while maintaining the constant area.  There are also
...
maker, \hl{please} see \citet{Chapin2013}. For more practical information
on using the software and the effect of different configurations in
practice, the Starlink Cookbook 21 \citet{SC21} should be consulted,
and for a detailed comparison between the configuration used here and
the Gould Belt Survey reduction, see \citet{Mairs2015}.
\hl{We probably need a brief summary here,
  including descriptions of the models.}
...
configuration parameters for this work.
...
Appendix~\ref{app:config}.  In this particular reduction,
after the initial pre-processing stage, the SCUBA-2 mapmaker recipe
performs five iterations in which the astronomical model (AST) is
retained at the end rather than being removed as in a normal
iteration. (This is specified using \texttt{ast.skip=5}). These
initial iterations allow the reduction process to focus on creating a
mask for the FLT model (which is used to filter the low frequency
noise) by identifying any unusually bright region; these regions may
cause ringing in subsequent estimates of the low-frequency noise in
each bolometer. During these iterations ``bright'' pixels are taken to
be those with a signal-to-noise ratio greater than 5.0, plus any other
...
additional 20 iterations containing all models). Each of these
remaining iterations removes the AST model of the astronomical signal
from the residuals prior to starting the next iteration. However, to
prevent instabilities in the iterative process, this subtraction only
occurs within regions corresponding to bright sources.  The AST mask
...
Every reduced observation suitable for inclusion in the co-adds was
...
that a lot of questionable observations required either checking of
the tile interactively or the manual making of an SNR image. For the
...
of completeness. All of these legacy-reduced observations are
publicly available through the JSA, regardless of our QA analysis
of them, so interested users can evaluate the full available data
themselves if they are unhappy with the co-add towards a tile.
...
\citep[][\ascl{1403.021}]{SUN139}. This co-adding was carried out as a
...
these were required. The variance map for the co-added tile is also
produced. The calibration from pW into m\jyas\ was done by multiplying
with an FCF of
...
point sources are better recovered.  This issue does in theory affect
all reductions, including those using external masks defined from
previous knowledge of the emission, but is of course most problematic
for reductions such as these where the AST mask is defined based only
on detected emission from a single observation.

If users of this release are concerned about the recovery of specific
sources, each individual reduced observation contains a bit mask
(stored in a FITS extension labeled QUALITY), indicating which areas
of the map were and were not included in the AST and FLT masks.
...
$\approx$0.02\,m\jyas. No processing was done to try and ameliorate
these effects, and there will be significant loss of detected emission
and omission of entire sources in the emission catalogs, due to these
artificial negative regions around bright sources. \hl{This could in
  theory be corrected for -- see Doug's note?}
...
60\arcsec, which is not done for our more-general-purpose data
reduction method. Based on these plots, we selected a source radius of
60\arcsec\ (chosen to avoid much of the negative bowling seen around
...
towards these three sources.\footnote{CRL 618 and CRL 2688
  ...
  included in this analysis.}  The Uranus observations from
...
packages.\footnote{The \texttt{autoastrom} command does not account for non-square
  pixels, so was not used for this analysis.}
 The KAPPA command \texttt{beamfit} was also used to fit the
amplitude of the beam, and this was used to calculate the beam
FCF. The Starlink FLUXES package \citet{2014ascl.soft05010J} was used to derived the expected flux
of Uranus at the time of each observation. For CRL\,618 and CRL\,2688,
the integrated and peak fluxes measured in \citet{Dempsey2013} were
used.
\begin{equation}
  \label{eq:1}
...
map, and $A$ is the area of a pixel in the ma/p
...
using a single aperture FCF. With Uranus being far brighter than
anything in our source emission, we have chosen to only use CRL\,618
and CRL\,2688 to derive our FCF value. We used the derived FCF from
each calibration observation described above, then applied an
iterative clipping which excludes FCF values 5-$\sigma$ away from the
mean (as extreme outliers will often be due to poor instrumental
performance, bad weather, day time observing or other affects that are
not present in the bulk of the released data.) No explicit cut was
made for the atmospheric transmission during the calibrator
observations, as no cut was made to the science observations included
in the co-adds. Figure \ref{fig:fcfhist} shows histograms of the full
distributions for each source.  The final average values found are:
...
\subsection{Effect on catalog fluxes.}
...
As such, we present the integrated and peak flux found within
...
for these effects or subtract any backgrounds. In practice, it should
...
individual observations that went into each mosaicked tile.
...
the peak pixel, the total flux contained within the entire extent, the
...
median radial offset of 2.86\arcsec{}. See Figure~\ref{fig:pointing}
...
science observations. When examining coadded science observations, a
correction factor for beam smearing may need to be included.
...
\begin{figure*}
  ...
  \caption{The SCUBA-2 850\micron\ beam, as found from a deep co-add
...
of CADC for their help with various technical aspects of this release.  \vspace{5mm}

\hl{Maunakea acknowledgement.}
...
\software{Starlink \citep{2014ASPC..485..391C,2011ascl.soft10012V},
  ...
  astropy \citep{2013A&A...558A..33A}, % units, fits, tables, 
  wcsaxes, % now in astropy core?
  healpy,
  pymoc}
