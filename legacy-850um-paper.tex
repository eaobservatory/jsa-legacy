...
co-adds and catalogs of all SCUBA data, and this resource has been
very widely used.
...
our tiles (see Section \ref{sec:healpix}), using the HPX
...
download through the JSA, where they will be listed under their
original project code and metadata. Pointing observations were not
used or analysed further in this release. We also excluded solar
...
include 12404 observations (5828 hours or 242.8 days) towards 4151
tiles. These co-adds were made up of 136 hours of science calibration
observations, 3575 hours of data taken for the JCMT Legacy Surveys and
2117 hours of data taken for PI projects. This information is
summarized in Table \ref{tab:typesobs}. The co-adds include 1.69
gigapixels of valid data, corresponding to $\approx$ 1356 square
degrees. For comparison, the SCUBA Legacy Catalog
\citep{DiFrancesco2008} covered 19.6 square degrees in their
`Fundamental Map Data Set, and 29.3 square degrees in their fuller
`Extended Map Data Set'.
...
we see towards bright sources -- clearly sen in the towards the
...
\begin{figure*}
  ...
  \caption{Examples of the co-add, noise map, and extents towards the
    ...
    $\approx$ 0.01 m\jyas\ bowling around the bright central source
    can be easily seen. }
  \label{fig:crl618}
\end{figure*}
...
\begin{figure*}
  ...
  \caption{Tile\,27258 showing an extremely deep co-add towards a
    ...
    here (shown as red crosses), as published in \citealt{Casey2013}
    (black circles) and as found by the JCMT CLS 850um DR1 \citep[purple
    triangles]{Geach2016}.}
  \label{fig:t27258}
\end{figure*}
...
tiles and pixels to grid the data onto.  The scheme needed to be
...
five iterations in which the astronomical model (AST)  is retained at the
...
the reduction process to focus on creating a mask for the FLT model
(which is used to filter the low frequency noise) by identifying any
unusually bright region; these regions may cause ringing in subsequent
estimates of the low-frequency noise in each bolometer. During these
iterations ``bright'' pixels are taken to be those with a
...
using \texttt{flt.zero\_snr=5} and \texttt{flt.zero\_snrlo=3}).
...
$\approx$0.01\,m\jyas. No processing was done to try and ameliorate
