...
\begin{abstract}
  ...
  uniformly reduced coadds and catalogs of detected emission for the
  ...
  easiest way to identify uniformly determined 850\,\micron\ detections
  and calibrated fluxes for any position observed by the
  ...
  local maxima were detected within these regions. Sixteen tils
  ...
  arcseconds. The coadded tiles include 5828 hours of observing time,
  covering 1356 square degrees of the sky. The coadds have been
  calibrated into units of m\jyas\ using a single, self-derived flux
  conversion factor of 2.46.

  The coadds and catalogues can be search at CADC at:
  ...
  can be downloaded from \url{INSERT\ URL\ HERE}. Masks of the full
  area included in this release, and the full area of detected
  emission are also available there.
...
paper presents the expanded and updated second legacy release, including SCUBA-2
...
It is expected that the 850\,\micron\ legacy release will be regularly updated
with new versions in the future, as more SCUBA-2 data is taken and
becomes public.
...
its modelled, see \citet{Chapin2013}, or for practical information on
using the software and the effect of different configurations in
practice, see Starlink Cookbook 21 \citet{SC21}. A detailed comparison
...
\hl{It should be emphasized that the data reduction configuration used
  in {Dempsey2013} was tuned for bright, isolated point sources
  and forces a flattening of the map at radii larger than 60\arcsec,
  which is not done in this more-general-purpose
  configuration.}
...
The FLUXES package \citet{2014ascl.soft05010J} was used to derived the
...
amplitude of the BEAMFIT Gaussian fit to the map, and $A$ is the area
of a pixel in the map ($3.22^2\,\textrm{arcsec}^{2}$).
...
\tablewidth{5in}
  \tablecolumns{3}
  \tablehead{ \colhead{} & \twocolhead{$S_{\mathrm{integ}}$}
    & \twocolhead{$S_{\mathrm{peak}}$}\\
    \colhead{}
    & \twocolhead{(Jy)}
    & \twocolhead{(Jy\,beam$^{-1}$)}}
  \startdata
...
% basic premise.
Our calibration scheme is based on the premise that from various
bright point sources of known flux we can derive a single FCF value
and that this will convert all other maps from instrumental pW into
flux densities, with the accuracy found from the uncertainty in our
FCF. However, our use of an auto-generated AST mask will alter the
relative flux recovery in our different calibrator sources, and we
here explore how this affects our calibration accuracy.
...
by \citet{Mairs2015}, the fluxes of sources outside an AST mask are
suppressed compared to sources inside the AST mask; this means that
...
\begin{figure}
  \centering
  \includegraphics[scale=0.8]{simulated-fcfs.pdf}
...
finds a correlation of $-0.44\ p=4E-122$ between the aperture FCF and
the 850\,\micron\ transmission; for comparison, the beam FCF (not
expected to be strongly affected by the AST masking) has a weaker
correlation of $-0.32\ p=6E-59$. Part of this correlation is probably
due to systematic errors in the transmission measurement, which affect
the extinction correction in \texttt{makemap}. The difference between
the beam and aperture FCF correlations may be due to the AST masking
effects.
...
\ref{tab:flux}. Taken together in quadrature, this gives an overall
