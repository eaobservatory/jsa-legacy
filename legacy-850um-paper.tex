...
\affiliation{East Asian Observatory, 660 N.\ A`oh\=ok\=u Place, Hilo, HI 96720, USA}
\affiliation{Joint Astronomy Centre, 660 N.\ A`oh\=ok\=u Place, Hilo, HI 96720, USA}
\author[0000-0001-6524-2447]{David S. Berry}
\affiliation{East Asian Observatory, 660 N.\ A`oh\=ok\=u Place, Hilo, HI 96720, USA}
\affiliation{Joint Astronomy Centre, 660 N.\ A`oh\=ok\=u Place, Hilo, HI 96720, USA}
\author[0000-0002-7316-4626]{Iain M. Coulson}
\affiliation{East Asian Observatory, 660 N.\ A`oh\=ok\=u Place, Hilo, HI 96720, USA}
\affiliation{Joint Astronomy Centre, 660 N.\ A`oh\=ok\=u Place, Hilo, HI 96720, USA}
\author[0000-0003-0141-0362]{Malcolm J. Currie}
...
\begin{abstract}
  ...
  uniformly reduced coadded tiles, and catalogs of detected emission,
  for the 850\,\um\ data from all SCUBA-2 observations taken between
  2011 February 2 and 2015 March 1.  This release provides the fastest
  and easiest way to identify uniformly determined 850\,\micron\
  detections and calibrated fluxes for any position observed by the
  JCMT. The coadded observations include 5828 hours of observing time
  and cover 1356 square degrees of the sky, with detections of
  contiguous areas of emission at better than 5~$\sigma$ covering 1.4
  square degrees of area. 13477 individual local maxima were detected
  within these regions. Sixteen tiles contain regions with a noise
  ...
  The coadds and catalogs can be searched and retrieved via the
  Canadian Astronomy Data Centre\footnote{\url{http://www.cadc-ccda.hia-iha.nrc-cnrc.gc.ca/en/search/?Observation.proposal.id=JCMT-LR}}.
...
coadded maps that require as little checking as possible by eye, and
which allow astronomers to easily discover which regions have been
observed, the noise levels in existing regions, and where there are
clear detections. Previously, the SCUBA (Submillimeter Common User
Bolometer Aray)Legacy Catalog \citep{DiFrancesco2008} presented
uniformly reduced coadds and catalogs of much of the public SCUBA data
at 850 and 450\micron, and this resource has been very widely used.
...
Planned future releases include the 450\,\um\ SCUBA-2 observations and
...
hosts the proprietary (with authentication) and public raw
observations, as well as the JCMT's standard pipeline reduced
products. Unlike the LR, the standard pipeline reductions do not
include full coadds of data taken on different nights. In addition,
observations of the same region but from different projects may have
been reduced with different configurations and cannot be naively
coadded.


\subsection{Outline of paper}

This paper presents in Section \ref{sec:overview} a summary of
the legacy release and the included observations, various overall statistics
...
example regions (chosen to cover a range of astronomical
...
summarizing the pointing offsets in Section \ref{sec:pointing},
...
observations taken between 2011 February 2 and 2015 March 1,
...
For these LR observations, we have produced individual reduced
...
The primary aims of this release are: to allow users to see where we
have observed; to identify the detected emission; and to get
consistently calibrated fluxes from the detected emission. In
particular, we wanted to make these tasks extremely \emph{easy} to carry out,
...
the researchers to reduce data from every project, instrument and
telescope themselves. Easy access to detected calibrated fluxes and
maps is also invaluable as pathfinders for follow-up observations (for
example, identifying sources for high-resolution follow up with the
...
in the CADC's multi-observatory archive \footnote{This is hosted at
  \url{http://www.cadc-ccda.hia-iha.nrc-cnrc.gc.ca/en/search}} (which
includes the JSA), and will be returned as appropriate in searches
based on spatial, temporal and spectral limits.
...
catalogs at once they can search for everything with a proposal ID of
`JCMT-LR'\footnote{\url{http://www.cadc-ccda.hia-iha.nrc-cnrc.gc.ca/en/search/?Observation.proposal.id=JCMT-LR&Observation.Collection=JCMT}}.
...
For convenience, we have also provided combined catalogs combining all
of the extent and peak catalogs respecitvely via a doi at:
\doi{INSERT\ URL\ HERE}. That page also contains spatial masks
...
include 12404 science and calibration observations which passed our quality testing
...
SCUBA Legacy Catalog \citep{DiFrancesco2008} at 850\micron\ covered 19.6 square
...
Sixteen of our coadded tiles contain pixels with a noise better
0.0043\,m\jyas{}, corresponding to 1\,m\jybm. These include our two
most common standard SCUBA-2 calibrators (CRL\,618 and CRL\,2688)
 and several deep cosmology fields that were observed
separately by both the JCMT Cosmology Legacy Survey and by various PI
projects. For reference, these sixteen tiles, along with some
...
range of non-solar-system structures observed by the JCMT -- including
dusty debris disks, complex filamentary molecular clouds, nearby
...
\subsection{Tile 30318: G034.27+0.15}
...
(Figure~\ref{fig:g34-3}, upper right) for this coadd shows the
...
below) indicates that this not prevent a high-SNR detection of the
emission in these regions. Our experience has been that as
higher-noise areas only occur towards extremely bright regions, they
are still clearly detected in our extent catalogs, as they have a
signal-to-noise ratio (SNR) of $>5$.
...
\begin{figure*}[htb!]
  ...
  \caption{The coadded products towards tile\,30318, containing the
    ...
    peaks within the detected extents. The extent and peak catalogs
    are described fully in Section \ref{sec:cat}. }
  \label{fig:g34-3}
\end{figure*}

\subsection{Tiles 1238 and 1244: CRL\,618}
...
detected in this deep coadd. In addition, with careful viewing by eye
there appear to be additional sources that are missed by our
...
\begin{figure*}[htb!]
  ...
  \caption{The coadd, noise map, and extents towards the
...
it can also be seen that there is no artificial structure appearing at
...
\citet{Casey2013}. Although these three methods do not detect exactly
the same objects, there are many correspondences between them. No
follow-up examination of the sources found only in our reduction has
been carried out.
...
HEALPix grid with our standard two-dimensional image array in FITS format. This
...
projection, the sources will appear distorted (or non-circular), and
grid lines will appear bent at discontinuities. This is due to
non-square pixels being represented as squres by the image rendering
software.
...
configuration for this legacy release. We found we could minimise the
blooms of fake emission by not attempting to recover the large scale
...
not select a more specific option based on their science goals or
expected emission structure.


These LR observations were reduced using Starlink, Starjava, and ORAC-DR
...
discussed in Appendix~\ref{app:config}. There are also a few specific
points to highlight. We explicitly filter out
...
the 25 iterations; these were still included in the coadds if they
passed our quality assurance.

\subsection{Quality Assurance (QA)}
%label{sec:QA}
Every reduced observation suitable for inclusion in the coadds was
assessed by eye, and not included in the coadds if it was considered low
...
interested users can individually evaluate each observation towards a
...
limits set by the specific configuration parameters chosen. The
magnitude of the relative reduction in the recovery of flux is
dependent on the size of the source -- extended sources outside of the
masked regions are more poorly recovered in this reduction, whereas point
sources are better recovered.  This issue does (in theory) affect all
\texttt{makemap} reductions, including those using external masks
defined from previous knowledge of the emission. However, it is more
problematic for reductions such as these, in which the AST mask is
defined based only on detected emission from a single observation.
...
Within the coadded maps of this release, negative bowling can be seen
around bright sources. Figure~\ref{fig:crl618} shows this around the
bright calibrator CRL\,618. The cuts in the vertical and horizontal
direction through the source show the bowling at
$\approx$0.02\,m\jyas{}, compared with a peak source brightness of
4.89\,\jybm{}. As the size scales of the negative bowling are
...
converting into mJy\,beam$^{-1}$ (a beam FCF). For this work, we derive both
...
FCF for this data set: Uranus, CRL\,618, and CRL\,2688.\footnote{CRL
  ...
  during QA (Quality Assurance) were not included in this analysis.}
The Uranus calibration observations are not part of the standard
legacy release themselves, as observations of moving targets were not
included in this release. They were reduced onto the pixels of the
same area as used in our HEALPix tiles, and using the same DR
configuration.
...
where $S_{\mathrm{integ}}$ is the known integrated flux in the source and
$S_{\mathrm{peak}}$ is the known peak flux of the source. $I_{0}$ is the summed
flux in the aperture of the map (minus the average flux per pixel in
the background annulus multiplied by the number of pixels in the
aperture), $I_{\mathrm{peak}}$ is the peak amplitude of the \texttt{beamfit}
Gaussian fit to the map, and $A$ is the area of a pixel in the map
...
\begin{figure*}
  ...
  \caption{Histograms of the derived beam and aperture FCFs
    separately for CRL 618, CRL 2688 and Uranus. The combined histogram
    for CRL 618 and CRL 2688 (i.e. the sources used to derive the
    calibration of the coadds) is shown as a grey background. The
    sigma-clipped mean for each source is shown as the solid vertical line,
    and the horizontal cross bar indicates the $\pm$ one standard
    deviation. The dashed vertical line indicates the median value.}
  \label{fig:fcfhist}
\end{figure*}
...
\begin{figure}
  ...
  \caption{A toy model illustrating the effect of source brightness on
    ...
    (extending down to a limit of 3 times the noise). Two one dimensional Gaussian
...
Our auto-generated AST mask follows sources down to an SNR of 3;
...
effect is illustrated in one dimension in Figure~\ref{fig:toymodel}.  As shown
by \citet{Mairs2015}, the fluxes outside an AST mask are suppressed
compared with fluxes inside the AST mask; this means that the emission
in the outer regions of the beam will be suppressed compared with the
central region. As Figure \ref{fig:toymodel} shows, an extremely
bright source (such as Uranus, with a flux 57.2--70.3\,Jy for these
observations) will have a higher percentage of its emission found
within the AST mask; a dimmer source (such as CRL 618 at 5.0\,Jy or
CRL 2688 at 6.13\,Jy) will have more of its emission in the non-AST
masked region where emission is suppressed. This will lead to a
systematic trend where, when comparing two observations with the same
noise, we will recover more of the flux for brighter sources -- and
hence derive a lower FCF. It can be seen in Figure \ref{fig:fcfhist}
that we do see a systematically lower aperture FCF distribution for
our Uranus observations. We also believe this variation in the AST
mask causes the very different curves of growth for Uranus as opposed
to CRL\,618 and CRL\,2688 (see Figure \ref{fig:cog}).
...
found a correlation of $-0.44$ (p-value=4$\times 10^{-122}$) between
...
has a weaker correlation of $-0.32$ (p-value=6$\times 10^{-59}$). Part
...
minimum uncertainty in this calibration. In addition, we need to
consider the uncertainty in the known sub-mm flux of Uranus (used to
bootstrap \citet{Dempsey2013}'s measurements, and quoted as $\pm$ 5
percent therein), and the uncertainty in the known fluxes of CRL\,618
and CRL\,2688 (see Table \ref{tab:fluxes}). Taken together in
quadrature, this gives an overall estimated uncertainty in calibrated
fluxes of $\pm$11 percent (aperture) and $\pm$12 percent (beam) due
only to the variation of these derived FCFs.

For repeatedly observed fields containing bright sources, it would be
possible for an interested user to substantially improve the
\emph{relative} (but not absolute) calibration accuracy within their
fields by comparing and scaling different individual observations,
then coadding them to form their own version of the LR tiles.
\citet{Mairs2017} have achieved \emph{relative} accuracies of 2-3\% on
SCUBA-2 data in this way, from an original measured relative flux
uncertainty of 8\% found in their observations. No attempt to apply
per-field calibration corrections was made in this release.

%Recent work by the Transient JCMT large program \citep{Mairs2017} has
%investigated improving the relative flux calibration of repeated
%observations of fields containing bright sources, and they have been able to achieve a relative calibration
...
the individual observations that went into each mosaicked tile.
...
implemented in the Starlink CUPID \citep{cupid} package for
...
emission with this simpler approach.
...
and to identify the position of the peak pixel within each map; these
outlines are available both as a FITS-format mask file, and as a
...
Appendix~\ref{app:example-cat}, Table~\ref{tab:extents}; the extents
found in that tile are shown in Figure \ref{fig:g34-3}.



\subsection{Peaks}
\label{sec:peaks}
In order to provide information about the nature of emission
...
catalog for tile\,30318 is shown in Appendix~\ref{app:example-cat},
Table~\ref{tab:peaks}; a plot of this catalog is shown in Figure
\ref{fig:g34-3}.

We chose not to include the `clump outline' of all the emission
associated with an individual peak, as we felt that would encourage
users to consider the peaks as being discrete physical
objects. Although they may be in some cases, in many other cases the
peak is simply a local maximum within a complex structure. The
`clump outlines' produced by algorithms such as FellWalker always
reflect the arbitrarily chosen boundaries of dips and minimum heights,
and should not be assumed to represent physical objects without
considerable further modeling.
...
science observations. When examining coadded science observations, if the
beam area is required to a high accuracy than a correction factor for
beam smearing may need to be included.
...
\ref{sec:calib}, we fitted a two-component beam model to the JCMT beam
shape, and found a very similar shape and size as found in
...
amplitude of 96 percent and 4 percent (compare with
\citealt{Dempsey2013} who found FWHM of 13.0\arcsec\ and 48\arcsec,
with relative amplitudes of 0.98 and 0.02). The effective FWHM beam is
found as 15.2\arcsec. Figure \ref{fig:beam} shows the image of the
deep Uranus coadded maps, as well as a comparison between the radial
profiles along the horizontal and vertical axes and the fitted beam
shape.
...
the JCMT Science Archive via CADC's multi-observatory search page
\footnote{\url{http://www.cadc-ccda.hia-iha.nrc-cnrc.gc.ca/en/search/?Observation.proposal.id=JCMT-LR&Observation.Collection=JCMT}}
...
found at \doi{INSERT\_DOI\_HERE}.
...
aperture Flux-Calibration-Factor (FCF) of 2.46\,Jy\,pW$^{-1}$\,arcsec$^{-2}$ to
convert all of our coadded tiles from instrumental pW to m\jyas\,
based on observations of the bright sources CRL\,618 and CRL\,2688 and
their known fluxes. We then discussed in detail some of the
limitations of this approach, caused by the details of the data
reduction configuration we used. This configuration was required to
allow disparate observations of e.g. blank fields or bright
filamentary clouds to be reduced with the same setup.
...
during the time range of this release (and reduced with the same
...
DJ is supported by the National Research Council of Canada and by an
...
\section{Map maker configuration}
\label{app:config}
The map maker configuration used for this release (known as a
\texttt{dimmconfig} file) is shown here. Like most SCUBA-2 map maker
...
the SCUBA-2 map maker recipe performs five iterations in which the
...
to filter the low frequency noise, see \citealt{Chapin2013}) by identifying any unusually bright
...
being introduced into the map; this can occur where the common modes vary
significantly, and are thus not well represented by a single common
...
&\textbullet{} Catalog of extents (see Sec.\,\ref{sec:extents})
