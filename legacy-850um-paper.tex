...
possible by hand, and which would allow astronomers to easily discover
...
Legacy Catalogue \citep{DiFrancesco2008} presented uniformly reduced
...
same configuration and co-add criteria as described in this paper, but
...
observing time, towards 4403 distinct tiles. The co-added
observations include 12404 observations (5828 hours or 242.8 days)
towards 4151 tiles. These co-adds were made up of 136 hours of science
...
and Jy\,beam$^{-1}$ using \hl{self-derived} FCFs (see section
...
it is possible to see the imprint of the original observations scan
...
To produce this uniform reduction of all public data, it was necessary to
...
maker, please see \citet{Chapin2013}. For more practical information
...
remaining iterations removes the astronomical signal from the
residuals prior to starting the next iteration. However, to prevent
instabilities in the iterative process, this subtraction only occurs
within regions corresponding to bright sources.  The mask identifying
such sources is created in the same way as the mask used by the FLT
...
\texttt{ast.zero\_snr=5} and \texttt{ast.zero\_snrlo=3}).  The map
maker exits before the 20th iteration if the \texttt{maptol} parameter
-- the change between maps -- has reached a mean value of 1\% of the
noise level (specified by \texttt{maptol=0.01}) Only 562 of the
science observations (4.4 percent) did not converge within the 25
...
first data release (2011 to 2013, not including the CLS) we only used a
