...
\begin{abstract}
  ...
  uniformly reduced co-additions of of all data that was public as of 2014 August
...
10686 observations were reduced, of which 5161 are non pointing
observations included in these co-additions and classified as \emph{science}
observations, and 5521 were pointing observations that were reduced
but not coadded.  Of these \emph{science} observations, 49
observations were marked as \status{BAD} and excluded from the co-additions
by our manual QA process.\footnote{A further 390 were marked as
...
\begin{table}
  ...
  {\centering
    \begin{tabular}{l r r r r}
      ...
      \hspace{0.5cm} Directors      &  57 &  57 &  60\% \\
      ...
      \hspace{0.5cm} Univ.~Hawaii   & 586 & 322 &  48\% \\
  ...
  {\noindent\scriptsize \textsuperscript{*} The percentage of all
    ...
    East Asian Observatories on 1 March 2015. This includes all data
...
5161 `science' observations were successfully reduced onto 2930
distinct  tiles, and 5519 `pointing' observations were reduced
individually onto 263 distinct HEALPix tiles.
...
'makemap'. In brief, this is an iterative process that divides the
data set into various instrumental and astronomical signals. There are
an extremely large number of user-adjustable parameters that can
...
compared to a galactic astronomer looking at bright, extended and
structurally complex emission from e.g. a molecular cloud. For full details on the SCUBA-2 map maker, please see \citet{Chapin2013}.
...
These standard configs can produce very poor results when used on an
inappropriate observation -- for example, the standard SCUBA-2 config
used for calibrator observations is tuned to expect a bright, compact
source at the centre of the map. If used by mistake on a blank
field this recipe could easily create \emph{fake emission} in the form
...
parties seeking to replicate these results can do so by using the STAR
version tagged as '2015A-legacy' release.\footnote{(See
...
HARRIET AND DAVID to WRITE THIS PROPERLY aND CHECK I HAVEN't MADE ANY MISTAKES!
...
region. This\todo{ Should this have a comparison with e.g. a GBS
...
\todo{Calibration numbers from Jess needed first!}
\begin{itemize}
...
the standard FCF values from \citet{Dempsey2013}.  As the effective
beam size is increased during the co-add procedure (due to errors in
the pointing between different maps), and the choice of pixel size
will also affect the effective beam size of the final map, we have
chosen not to try and calibrate into units of the beam.
...
modelling (\todo{SFG:Cite and further discuss the various papers highlighting the problems in clump numerology -- list is on laptop}.)
