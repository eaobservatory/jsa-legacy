...
\begin{abstract}
  ...
  SCUBA-2 850\,\um\ observations. These reductions provide uniformly
  reduced co-additions of of all standard, public data observed before
  2015 March 1st, as well as catalogues to identify the regions where
  emission was detected. The data have been gridded in the HEALPix HPX
  scheme with pixels of area 3.22 arcsec\textsuperscript{2}, and
  divided into tiles of just under 1 degree a side.
\end{abstract}
...
is summarized in Table \ref{tab:typesobs}. The coadds include 1.69
gigapixels of valid data, corresponding to ~1356 square
degrees.\footnote{\note{1.69e9 * (3.22/(60.0*60.0)**2?}}.  Source
...
each pixel of the coadded tiles. Table~\ref{tab:noises} shows the area
...
dividing by our derived arcsecond FCF (2.48\,\jyas\,pW$^{-2}$) and
multiplying by the JCMT `standard' beam FCF (537
Jy\,beam$^{-1}$\,pW$^{-1}$. Please note that a beam FCF has not been
...
Twenty of our coadded tiles contain pixels with a noise better than
...
\begin{deluxetable}{rc r c c p{2cm}}
  \tablecaption{The twenty coadded tiles which contain regions with a
...
% to examine the coadds and the catalogs.  Includes: filamentary
...
% deep submm galaxies when coadded.
...
\begin{figure*}
  ...
  \caption{Examples of the coadd, noise map, and extents towards the
    ...
    3e-5, 4e-5 and 5e-5 m\jyas\ noise
    ...
    cut throught the ~0.01 m\jyas\ bowling around the bright central source
    can be easily seen. }
  \label{fig:crl618}
\end{figure*}
...
using `detection' limits setin the specific configuration parameters
...
in practice affect all reductions, including those using external
masks defined from previous knowledge of the emission, but is of
course most problematic for reductions such as these where the AST
mask is defined based only on detected emission from a single
observation.
...
which indicating which areas of the map were included in the AST mask
(and also the FLT mask).
...
effects, and there may be significant loss of detected emission in the
cataloging stage due to these artificial negative regions.
...
reductions and removing them from the coadds.
...
modelling (\todo{Does this need further discussion? Cite various issue with clumpfind papers maybe?}.)
...
pointing-corrected coadds, a correction factor of 2.9\arcsec\ beam
...
EAO operates the JCMT since March 1st 2015, on behalf of The
...
%
Coadded tile &  COADD\_JSA\_TILES  & \texttt{jcmts850um\_healpix<TILE>\_pub\_001.fits} &
...
\tablecomments{In the filename column, \texttt{<SCAN>} indicates the
  JCMT scan number of that observation, padded with zeros to 5 digits
